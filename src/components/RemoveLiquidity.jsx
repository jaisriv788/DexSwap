...
function RemoveLiquidity({ setShowRemoveModal, setClickedPool, data }) {
  ...
  async function handleRemoveLiquidity() {
    ...
    try {
      ...
      const tx1 = await lpToken.approve(ROUTER_ADDRESS, liquidity);
      await tx1.wait();

      const router = new ethers.Contract(ROUTER_ADDRESS, routerAbi, signer);
      //   const tokenOneValue = parseUnits(tokenOneAmount);
      //   const tokenTwoValue = parseUnits(tokenTwoAmount);
      const amountAMin = 0;
      // tokenOneValue - (tokenOneValue * BigInt(SILP_PAGE)) / BigInt(100);
      const amountBMin = 0;
      // tokenTwoValue - (tokenTwoValue * BigInt(SILP_PAGE)) / BigInt(100);
      const to = address;
    ...
    >
      ...
      >
        ...
        <button
          ...
          className="relative overflow-hidden w-full rounded-lg py-2.5 font-medium text-white 
